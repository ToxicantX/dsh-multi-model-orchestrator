...
test('role guidance uses available capacity without duplicating or inventing work', () => {
  ...
  assert.match(text, /exclusively owns its assigned scope and acceptance target until it settles/)
  assert.match(text, /Never implement or run equivalent tests for a target a child owns/)
  assert.match(text, /Continue only clearly non-overlapping management or integration work/)
