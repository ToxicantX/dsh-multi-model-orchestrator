...
test('role guidance uses available capacity without duplicating or inventing work', () => {
  ...
  assert.match(text, /Read child status literally: running means the child is actively executing and must be allowed to continue/)
  assert.match(text, /idle or ready means no turn is active and is not evidence of either completion or a deadlock/)
  ...
  assert.match(text, /Only classify a child as stuck when there is concrete evidence/)
  assert.match(text, /two status checks show no progress together with an unresolved blocker/)
  ...
  assert.match(text, /Before interrupting, state the evidence and target only the direct child/)
  assert.match(text, /interrupt_agent stops the current turn but preserves the resumable child/)
  ...
  assert.match(text, /Do not finalize while a required child is still running or its task has no returned result/)
