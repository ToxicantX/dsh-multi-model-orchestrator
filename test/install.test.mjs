...
test('installer copies exactly the fixed orchestrator preset and no agent data rows', async () => {
  ...
  try {
    ...
    assert.match(output, /exclusively owns its assigned scope and acceptance target until it formally returns/)
    ...
    assert.match(output, /Interrupt a child only for user cancellation, an explicit deadline, a confirmed deadlock, or verified repeated tool failure/)
