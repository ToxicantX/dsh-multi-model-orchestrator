...
test('installer copies exactly the fixed orchestrator preset and no agent data rows', async () => {
  ...
  try {
    ...
    assert.match(output, /Treat up to three configured development specialists as reusable specialist tools, not as a limit on children or tasks/)
    ...
    assert.match(output, /Decompose work by complexity into any number of meaningful, non-overlapping tasks with independent acceptance criteria/)
    assert.match(output, /Avoid artificial or overly granular splits/)
    assert.match(output, /Map each task to the best-fit available specialist/)
    assert.match(output, /Reuse the same specialist tool across calls; create a distinct child for each task/)
    assert.match(output, /Dispatch independent tasks concurrently; runtime capacity, rather than specialist roster size, is the constraint/)
    assert.match(output, /Do not keep a suitable specialist idle while you perform development/)
    assert.match(output, /only when no meaningful matching task exists or its work depends on unfinished results/)
    assert.match(output, /Never invent work merely to use every specialist/)
    assert.match(output, /Each child exclusively owns one cohesive task and one acceptance target until it formally returns/)
    assert.match(output, /Serialize tasks that overlap files, ownership, or dependencies/)
    assert.match(output, /Never implement or run equivalent tests for a target a child owns/)
    assert.match(output, /Continue only clearly non-overlapping management work/)
    assert.match(output, /Before every analysis, edit, or test step, re-check whether any child owning a related scope is still running/)
    assert.match(output, /hard phase barrier/)
    assert.match(output, /Read child status literally: running means the child is actively executing and must be allowed to continue/)
    assert.match(output, /idle or ready means no turn is active and is not evidence of either completion or a deadlock/)
    assert.match(output, /After any child returns, do not advance immediately if another child is still running/)
    assert.match(output, /status is missing, partial, or ambiguous/)
    assert.match(output, /Only classify a child as stuck when there is concrete evidence/)
    assert.match(output, /two status checks show no progress together with an unresolved blocker/)
    assert.match(output, /do not begin integration or final acceptance until every relevant child has formally returned/)
    ...
    assert.match(output, /Reuse the same continuable child for follow-up corrections to the same task; create a new child for each new independent task, even when it maps to the same specialist/)
    ...
    assert.match(output, /Use foreground child calls \(set run_in_background: false\) when no independent work remains/)
    assert.match(output, /Do not repeatedly call list_agents just to pass time/)
    assert.match(output, /Several minutes of a child running alone is not a stall/)
    assert.match(output, /Treat a child that is still running and has not reported an error as healthy/)
    assert.match(output, /Elapsed time, silence, repeated or unchanged status, or another child finishing are never, alone or together, evidence of a deadlock/)
    assert.match(output, /Interrupt a child only for direct user cancellation, an explicit deadline that has arrived, a confirmed deadlock supported by concrete evidence, or verified repeated tool or execution failure/)
    assert.match(output, /Never call interrupt_agent to request an early report, shorten a wait, regain control, start integration, or avoid waiting/)
    assert.match(output, /Do not infer a deadlock from duration or lack of messages/)
    assert.match(output, /Before interrupting, state the evidence and target only the direct child/)
    assert.match(output, /interrupt_agent stops the current turn but preserves the resumable child/)
    assert.match(output, /send_message queues the next turn and does not redirect current work/)
    assert.match(output, /Specialists stop after two occurrences of the same tool or execution-protocol error/)
    assert.match(output, /Switch to the simplest valid alternative tool call or report the blocker/)
    assert.match(output, /avoid repeated calls that fail or produce no useful output/)
    assert.match(output, /Do not finalize while a required child is still running or its task has no returned result/)
    assert.doesNotMatch(output, /configured roster of up to three development specialists as available execution capacity/)
    assert.doesNotMatch(output, /different specialist for each concurrently executable scope/)
    assert.doesNotMatch(output, /up to the available specialist count/)
    assert.doesNotMatch(output, /three-Agent limit/)
    const primarySource = await readFile(new URL('../preset/agent.cordis.yml', import.meta.url), 'utf8')
    const legacySource = await readFile(new URL('../preset-legacy/agent.cordis.yml', import.meta.url), 'utf8')
    const persona = text => text.match(/^- id: persona[\s\S]*?(?=^- id: agent-instructions)/mu)?.[0]
    assert.ok(persona(primarySource))
...
test('normalizeAgents permits empty only with allowEmpty', () => {
  ...
  assert.equal(normalizeAgents([{ id: 'defaulted', provider: 'p', model: 'm' }])[0].persona, undefined)
  assert.equal(normalizeAgents([agent('custom', { persona: 'Use the dedicated child prompt.' })])[0].persona, 'Use the dedicated child prompt.')
...
test('normalizeAgents preserves internal line breaks in multiline descriptions', () => {
  ...
  assert.throws(() => normalizeAgents([agent('multi', { persona: 'x'.repeat(8001) })]), /exceeds 8000 characters/)
})
